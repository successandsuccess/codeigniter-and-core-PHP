...
var fmLib =  {
	...
	sortJson: function(json, field, order) {
		...
		items.sort(function(a, b) {
			...
			if(field == 'isDir') {
				prefix = a.isDir ? 'a' : 'z';
				str1 = prefix + a.name.toLowerCase();
				prefix = b.isDir ? 'a' : 'z';
				str2 = prefix + b.name.toLowerCase();
			}
			else if(field == 'size') {
				str1 = fmTools.toBytes(a.size);
				str2 = fmTools.toBytes(b.size);
			}
			else if(field == 'changed') {
				str1 = a.changedTimeStamp;
				str2 = b.changedTimeStamp;
			}
			else {
	...
	playVideo: function(url, name, curCont) {
		...
		if(name.match(/\.swf$/i)) {
			...
			if(typeof FlashReplace == 'object') {
				params = {FlashVars: 'auto_play=true&fichier=' + encodeURIComponent(url), bgcolor: '#000000', allowFullScreen: 'true'};
