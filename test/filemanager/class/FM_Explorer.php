...
/* PROTECTED METHODS *************************************************************************** */
	...
	protected function _makeContent() {
		if(is_array($this->getFolders())) {
			...
			foreach($this->_folders as $id => $Folder) {
				...
				$item .= "size:'" . FM_Tools::bytes2String($Folder->size) . "'}";
				$items[] = $item;
			}
	...
	protected function _makeFooter() {
		return '],totalSize:' . (float) $this->_totalSize . '}';
	}
	...
	protected function _readFolder($dir) {
		...
		if(is_array($items)) foreach($items as $item) {
			if(!$item['isDir'] && !$item['isDeleted']) {
				$ext = strtolower(FM_Tools::getSuffix($item['name'], '.'));

				/* check if file extension is allowed */
				if($ext != '') {
					$hidden = $this->_FileManager->hideFileTypes;
					$allowed = $this->_FileManager->allowFileTypes;
		...
		return array($items, $cntFiles, $size);
	}
	...
	protected function _readCache() {
		$content = FM_Tools::readLocalFile($this->_cacheFile);
		if($content) {
			list($size, $folders) = unserialize($content);
			if(is_int($size) && is_array($folders)) {
				$this->_totalSize = $size;
				$this->_FileManager->explorerFromCache = true;
				return $folders;
			}
		}
		return false;
	...
	protected function _writeCache() {
		if($this->_cacheFile != '') {
			$content = array($this->_totalSize, $this->_folders);
			FM_Tools::saveLocalFile($this->_cacheFile, serialize($content));
			$this->_lastCacheUpdate = time();
		}
	}
}
