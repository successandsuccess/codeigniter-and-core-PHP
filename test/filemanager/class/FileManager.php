...
if($memoryLimit && FM_Tools::string2bytes($memoryLimit) < 134217728) {
...
/* PUBLIC PROPERTIES *************************************************************************** */
	...
	/**
	 * true if directory tree has been loaded from cache
	 *
	 * @var boolean
	 */
	public $explorerFromCache;
...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function create() {
		...
		// print_r($this->tmpDir); exit;

		if(!is_writable($this->tmpDir)) {
			$msg = 'FileManager cannot use tmp directory! ';

			if(is_writable('/tmp')) {
				$msg .= 'Set variable tmpFilePath to "/tmp".';
			}
			else if(is_writable('C:/Windows/temp')) {
				$msg .= 'Set variable tmpFilePath to "C:/Windows/temp".';
			}
			else $msg .= 'Check variable tmpFilePath.';
	...
	public function action() {
		...
		switch($fmMode) {
			...
			default:
			// print_r($this->ftpHost); exit;
				if(!$this->ftpHost && $this->rootDir == '') {
					FileManager::error('SECURITY ALERT: Please set a start directory or an FTP server!');
				}
...
/* PUBLIC STATIC METHODS *********************************************************************** */
	...
	public static function error($msg) {
		...
		$d = @date('Y-m-d H:i:s');
		$m = addslashes($msg);
		die(htmlspecialchars("{cont:'$cont',lang:'en',title:'ERROR',error:'$m',messages:[{time:'$d',text:'$m',type:'error'}],end:1}"));
	}
...
/* PROTECTED METHODS *************************************************************************** */
	...
	protected function _getDebugInfo() {
		...
		$json .= "memoryLimit:'" . FM_Tools::bytes2string(FM_Tools::string2bytes(@ini_get('memory_limit')), 0) . "',";
		...
		$json .= "curDir:'" . ($this->_Listing ? addslashes(FM_Tools::utf8Encode($this->_Listing->curDir, $this->encoding)) : '') . "',";
		$json .= "search:'" . ($this->_Listing ? addslashes(FM_Tools::utf8Encode($this->_Listing->searchString, $this->encoding)) : '') . "',";
		$json .= 'cache:' . FM_Tools::getFileCount($this->getContCacheDir(false)) . ',';
		$json .= "explorerFromCache:'" . ($this->explorerFromCache ? 'yes' : 'no') . "'}";
		return $json;
	}
	...
	protected function _checkVars() {
		...
		if($this->quota != '') {
			$this->quota = FM_Tools::string2bytes($this->quota);
		}
	}
}
