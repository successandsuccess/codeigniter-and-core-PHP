...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function view() {
		...
		if(!empty($this->_entries[$this->_folderId]) && is_array($this->_entries[$this->_folderId])) {
	...
	public function upload($src, $newName, array &$info = null) {
		...
		if($this->FileManager->hideSystemFiles && $newName[0] == '.') {
			$this->FileManager->Log->add("Access denied: $newName", 'error');
			$info['errors'][] = FM_Tools::getMsg('errAccess') . ": $newName";
			return false;
		}
		...
		if($this->FileManager->quota > 0 && is_file($src)) {
			if($this->Explorer->getTotalSize() + @filesize($src) > $this->FileManager->quota) {
				$this->FileManager->Log->add('Quota exceeded', 'error');
				$info['errors'][] = FM_Tools::getMsg('errQuota');
				return false;
			}
		}
		...
		if($this->FileManager->maxImageWidth > 0 || $this->FileManager->maxImageHeight > 0) {
			if(in_array($ext, array('jpg', 'jpeg', 'png', 'gif'))) {
				if($Entry = new FM_Entry($this, true)) {
					...
					if($error = $Image->save()) {
						$this->FileManager->Log->add($error, 'error');
						$info['errors'][] = $error;
					}
				}
			}
...
/* PROTECTED METHODS *************************************************************************** */
	 ...
	 */
	protected function _addEntry(array $file, $dir = '') {
		...
		if(is_object($Entry)) {
			...
			if($Entry->isDir()) {
				...
				/* check if file extension is allowed */
				$name = preg_replace('/\.' . FM_EXT_DELETED . '$/', '', $Entry->name);
				$ext = strtolower(FM_Tools::getSuffix($name, '.'));
	 ...
	 * @return mixed				entry object, directory name or false
	 */
	protected function _createEntry(array $file, $dir) {
		$Entry = new FM_Entry($this);
