...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function getSystemType() {
		...
		if($this->_sysType) {
			$this->isWindows = (strtoupper(substr($this->_sysType, 0, 3)) == 'WIN');
...
/* PROTECTED METHODS *************************************************************************** */
	 ...
	 *
	 * @param string $path		file path
	 * @return string			encoded/decoded file path
	 */
	protected function _checkEncoding($path) {
		$parts = explode('/', $path);

		foreach($parts as $key => $val) {
			$parts[$key] = $this->useUtf8
						 ? FM_Tools::utf8Encode($val, $this->_FileManager->encoding)
						 : FM_Tools::utf8Decode($val, $this->_FileManager->encoding);
		}
