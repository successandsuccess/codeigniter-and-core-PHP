...
/* PUBLIC METHODS ****************************************************************************** */
	...
	public function read() {
		if($this->_ftp) {
			...
			try {
				$list = new DirectoryIterator($this->_path);
			}
			catch(Exception $e) {
				return false;
			}
		...
		if($list) {
			...
			foreach($list as $file) {
				...
				if($this->_ftp) {
					if(!$this->_isWindows && preg_match(self::UNIX_ROW, $file, $m)) {
						if($m[7] == '..' || $m[7] == '.') continue;
					...
					else if($this->_isWindows && preg_match(self::WINDOWS_ROW, $file, $m)) {
						if($m[4] == '..' || $m[4] == '.') continue;
						$t = explode(':', $m[2]);
						...
						$item['changed'] = ($tstamp > 0) ? @strftime($this->_FileManager->dateTimeFormat, $tstamp) : $m[1] . ' ' . $m[2];
						$item['permissions'] = '';
						$item['owner'] = '';
						$item['group'] = '';
					...
					$item['owner'] = $file->getOwner();
					$item['group'] = $file->getGroup();
					$item['size'] = $file->getSize();
					$item['changed'] = @strftime($this->_FileManager->dateTimeFormat, $file->getMTime());
					$item['name'] = $file->getFilename();
					$item['permissions'] = FM_Tools::getPermissions($this->_path . '/' . $item['name']);
					$item['target'] = is_link($file->getPathname()) ? @realpath($file->getPathname()) : '';
				}
