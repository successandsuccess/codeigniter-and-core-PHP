...
/* PROTECTED METHODS *************************************************************************** */
			...
			if(is_array($fmFile)) {
				for($i = 0; $i < count($fmFile['size']); $i++) {
					...
					if($fmFile['size'][$i]) {
						...
						$info = '';
						foreach($conf as $key => $val) $info .= "$key = $val<br />";
						$this->_FileManager->Log->add("Could not upload $file<br />$info", 'error');
						$uploaded['errors'][] = FM_Tools::getMsg('errSave', "$file<br />$info");
					}
				}
			}
	...
	protected function _saveFromUrl($url) {
		...
		if(!$this->_Listing->FileSystem->saveFromUrl($url, $this->_FileManager->getUserTmpDir())) {
			$uploaded['errors'][] = FM_Tools::getMsg('errOpen', $newFile);
		}
		else if(($newFile = $this->_Listing->upload($src, $newFile, $uploaded)) === false) {
			$uploaded['errors'][] = FM_Tools::getMsg('errSave', $newFile);
		}

		if($this->_FileManager->mailOnUpload && $uploaded['files']) {
			$this->_FileManager->sendUploadInfo($uploaded['files']);
		}
		if($uploaded['errors']) $this->_Listing->view();
		else $this->_Listing->refresh($this->_Listing->curDir);
		return implode('<br />', $uploaded['errors']);
	}
	...
	protected function _editFile($id) {
		if($this->_FileManager->enableEdit && $id != '') {
			if($Entry = $this->_Listing->getEntry($id)) {
				if(!empty($_POST['fmText'])) {
					$fmText = $_POST['fmText'];
					$data = FM_Tools::readLocalFile($Entry->getFile());
					if(!FM_Tools::isUtf8($data)) {
						$encoding = FM_Tools::getEncoding($data);
	...
	protected function _getContSettings() {
		...
		print 'fmHeight:' . (strstr($this->_FileManager->fmHeight, '%') ? "'" . addslashes($this->_FileManager->fmHeight) . "'" : (int) $this->_FileManager->fmHeight) . ',';
		...
		print "docViewerUrl:'" . addslashes($this->_FileManager->docViewerUrl) . "',";
		...
		if($this->_FileManager->enableDownload) {
			...
			if(count($dirs) > 0) foreach($dirs as $Entry) {
				if($this->_Listing->readDir($Entry->path)) {
					...
					if(is_array($entries)) {
						$newIds = array();
						foreach($entries as $SubEntry) $newIds[] = $SubEntry->id;
						$files = array_merge($files, $this->_getFiles($newIds));
					}
				}
			}
		}
	...
	protected function _sendZip($entries) {
		if(is_array($entries) && count($entries) > 0) {
			$downloads = array();
			$memoryLimit = FM_Tools::string2bytes(@ini_get('memory_limit'));
