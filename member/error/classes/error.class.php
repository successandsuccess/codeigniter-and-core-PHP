...
class ErrorObject
{
    ...
    private $table_name = "errorsubmission";
    ...
    {
        $check = "SELECT * FROM " . $this->table_name . " WHERE issue = '" . $this->issue . "' AND title = '" . $this->title . "'";
        $check_count = $this
            ->conn
            ->getCount($check);
        if ($check_count > 0)
        {
            $query = "UPDATE " . $this->table_name . " SET addeddate = '" . $this->addeddate . "', uname = '" . $this->uname . "', title = '" . $this->title . "', issue = '" . $this->issue . "', publish = '" . $this->publish . "' WHERE issue = '" . $this->issue . "' AND title = '" . $this->title . "'";
        }
        else
        {
            $query = "INSERT INTO " . $this->table_name . " (addeddate, uname, title, issue, publish) VALUES ('" . $this->addeddate . "', '" . $this->uname . "','" . $this->title . "','" . $this->issue . "','" . $this->publish . "') ";
        }
        if ($this
            ->conn
            ->execute($query))
    ...
    {
        $query = "SELECT * FROM " . $this->table_name . " WHERE publish = 'Yes' ORDER BY addeddate DESC LIMIT {$from_record_num}, {$records_per_page}";
    ...
    {
        $query = "SELECT * FROM " . $this->table_name . " ORDER BY addeddate DESC";
    ...
    // read data by uname
    public function readByAuthor($uname = "")
    {
        if ($uname != "") $this->uname = $uname;
    ...
    {
        $query = "UPDATE " . $this->table_name . " SET status = '" . ($this->status) ."' , fix_content = '". ($this->fix_content) ."' WHERE error_id = " . $this->id;
    ...
    public function delete()
    {
        if ($this
            ->conn
            ->delete_error($this->id, $this->table_name))
        {
            return true;
        }
        else
...
EOD;
    ...
    public function output_single($id = "")
    {
        ...
        if (($results) && (is_array($results)))
        {
            ...
            $title = $rec["title"];
            $uname = $rec["uname"];
            $addeddate = date("m/d/Y", strtotime($rec["addeddate"]));
            $issue = $rec["issue"];
            if (empty($this->viewPhoto($post_id)) == false)
            {
                $uname_photo1 = $this->viewPhoto($post_id);
                $uname_photo = $uname_photo1[0]['photo'];
            }
            else
            {
                $uname_photo = "./assets/images/ceo.png";
            }
            $user_id = $_SESSION["user_id"];
            $user_likes = $this->getUserLike($user_id);
            if ($user_likes) $like_class = " active ";
            else $like_class = "";
            $trimedstring = trim(preg_replace('/\s\s+/', ' ', $issue));
            // print_r($uname); exit;
            if ($uname == "Megan Varellas")
            {
                $outputHTML .= <<<EOD
                                                ...
                                                <div class="blog-main block-border ncca-right-padding">
                                                    ...
                                                    <div class="blog-main-container" style="margin-top:15px;">
                                                        ...
                                                        <div class="uname-time row">


                                                            <div class="col-9">


                                                                <img src="{$uname_photo}" alt="" class="img-fluid uname-photo">


                                                                <span class="uname-name"><h4> {$uname} </h4><p>NCCAA Board Chair</p></span>
                                                    ...
                                                    <div class="blog-main-container" style="margin-top:15px;">
                                                        ...
                                                        <div class="uname-time row">
                                                            ...
                                                            <div class="col-9">
                                                                ...
                                                                <span class="uname-name"><h4> {$uname} </h4><p>NCCAA CEO</p></span>
            ...
            foreach ($result as $rec)
            {
                ...
                $outputHTML .= <<<EOD
                            ...
                            <tr>
                                ...
                                <td><a href="?action=viewuname&uname={$uname}"> {$uname} </a></td>
...
EOD;
            foreach ($results as $rec)
            {
                ...
                $uname = $rec["uname"];
                $addeddate = date("m/d/Y", strtotime($rec["addeddate"]));
                $issue1 = strip_tags($rec["issue"], "<p>");
                $issue = substr($issue1, 0, 380);
                if (empty($this->viewPhoto($post_id)) == false)
                {
                    $uname_photo1 = $this->viewPhoto($post_id);
                    $uname_photo = $uname_photo1[0]['photo'];
                    ...
                    $outputHTML .= <<<EOD
                                                        ...
                                                        <div class="uname-time row">
                                                            ...
                                                            <div class="col-12" style="display: inherit;">
                                                                ...
                                                                <div class="img-fluid custom-oval-css uname-photo"  style="background-image: url('{$uname_photo}'); background-size: 90px;  background-repeat: no-repeat; background-position: center;">
                                                                
                                                                </div>


                                                                <span class="uname-name2"><h4 style="font-size:20px;"> {$uname} </h4><p>NCCAA Board Chair</p></span>
                    ...
                    $outputHTML .= <<<EOD
                                                        ...
                                                        <div class="uname-time row">
                                                            ...
                                                            <div class="col-12" style="display: inherit;">
                                                                ...
                                                                <span class="uname-name2"><h4 style="font-size:20px;"> {$uname} </h4><p>NCCAA CEO</p></span>
...
EOD;
    ...
    {
        // add user picture
        if (empty($file['add_picture_file']["name"]) == false && empty($data['uname']) == false)
        {
            $query = "SELECT * FROM " . $this->photo_table_name . " WHERE user_id = " . $data['user_id'] . " AND full_name = '" . $data['uname'] . "'";
            if ($user_count = $this
                ->conn
                ->getCount($query))
            {
            ...
            if ($user_count > 0)
            {
                $query1 = "SELECT * FROM " . $this->photo_table_name . " WHERE user_id = " . $data['user_id'] . " AND full_name = '" . $data['uname'] . "'";
                $stmt1 = $this
                    ->conn
                    ->getData($query1);
                ...
                $query = "UPDATE " . $this->photo_table_name . " SET photo = '" . $this->photo_dir . "', addeddate = '" . date('Y-m-d') . "' WHERE user_id = " . $data['user_id'] . " AND full_name = '" . $data['uname'] . "'";
            }
            else
            {
                $query = "INSERT INTO " . $this->photo_table_name . " (user_id, full_name, photo, addeddate) VALUES('" . $data['user_id'] . "', '" . $data['uname'] . "', '" . $this->photo_dir . "', '" . date('Y-m-d') . "')";
            }
            if ($stmt = $this
                ->conn
                ->execute($query))
    ...
    {
        $uname_sql = "SELECT uname FROM " . $this->table_name . " WHERE id = " . $id;
        if ($uname_name = $this
            ->conn
            ->getData($uname_sql))
        {
            $photo_sql = "SELECT * FROM " . $this->photo_table_name . " WHERE full_name = '" . $uname_name[0]['uname'] . "'";
            if ($photo_name = $this
                ->conn
                ->getData($photo_sql))
            {
