...
if(isset($_POST['employer_compensation_submit'])){
	...
	$overtime_compensation = $_POST['overtime_compensation'];
	 ...
	 $query = "select * from final_employer_compensation where user_id=".$_SESSION['user_id'];
    ...
    if($rowcount >0){
       ...
       $update_query = "UPDATE `final_employer_compensation` SET `basic_compension`='".$allcompensation_box."',`full_compension`='".$all_full_time_compensation_box."',`overtime_pay`='".$considered_overtime_pay."',`overtime_compensation`='".$overtime_compensation."' WHERE user_id=".$_SESSION['user_id'];
