...
if(isset($_POST['other_membership_submit'])){
	...
	$query = "select * from final_other_memberships where user_id=".$_SESSION['user_id'];
    ...
    if($rowcount >0){
       ...
       $update_query = "UPDATE `final_other_memberships` SET `belong_to_othermembership`='".$other_membership."',`dated_joined`='".$membership_expiry_date."',`membership_number`='".$membership_no."',`other_groups_belongs`='".$member_groups."',`comments`='".$memberships_comments."' WHERE user_id=".$_SESSION['user_id'];
