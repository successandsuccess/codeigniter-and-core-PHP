...

{
...

    {
...

        {
            ...
            $query = "SELECT * FROM temp_stats WHERE YEAR(cdq_due) = " . $year . " OR YEAR(cdq_due) = " . ($year + 10);
     ...
     * @return array
     */
    public function get_cert_registration_export_data($year) {
        $query = "SELECT A.*, B.action_date, B.amount_num, B.exam_mon, B.exam_year, C.role, C.full_name, C.email,
                    D.University_Name, E.Expected_Graduation, E.Expected_Cert_Exam, F.actual_certification_exam_taken
                    FROM
                    (SELECT * FROM tbl_class WHERE class_of = $year GROUP BY user_id) 
                    AS A LEFT JOIN (SELECT * FROM action_history WHERE exam_year = $year AND action_num > 2 AND exam_type = 'Certification') AS B 
                    ON A.user_id = B.user_id LEFT JOIN (SELECT id, role, full_name, email FROM users) AS C ON A.user_id = C.id
                    LEFT JOIN (SELECT id, University_Name FROM tbl_university) AS D ON A.University_id = D.id
                    LEFT JOIN (SELECT University_Id, Expected_Graduation, Expected_Cert_Exam FROM tbl_expected_dates WHERE class_of_year = $year) AS E ON A.University_Id = E.University_Id
                    LEFT JOIN (SELECT user_id, actual_certification_exam_taken FROM final_exam_certification_info) AS F ON (B.user_id = F.user_id AND B.exam_mon = MONTH(F.actual_certification_exam_taken)
                     AND B.exam_year = YEAR(F.actual_certification_exam_taken)) GROUP BY A.user_id";
