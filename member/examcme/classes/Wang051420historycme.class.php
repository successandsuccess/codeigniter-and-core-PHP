...
class PaymentHistoryCME
{
    ...
    public function insert_CMEActionHistory($data)
    {
        ...
        if ($check_cme_paid > 0) {
            ...
            // global $storedCmeIds;
            // if(count($storedCmeIds) > 0)
            // {
            //     foreach($storedCmeIds as $storedCmeId)
            //     {
            //         $query_update_cme = "UPDATE tbl_add_cme SET checked = 1 WHERE id=".$storedCmeId;
            //         $this->conn->execute($query_update_cme);
            //     }
            // }
            // $storedCmeIds = array();
            ...
            $this->conn->execute($query_checked);
