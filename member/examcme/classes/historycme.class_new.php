...
class PaymentHistoryCME
{
    ...
    private $table_add_cme = "tbl_add_cme";
    ...
    public function readActionContent($id)
    {
        $content = array(
            ...
            "CME Late Payment Received Receipt"
            //*************2  e.g.(late fee)6/17/20
    ...
    public function pay_title($id)
    {
        ...
        //extend late payment for cmes exam on 2020/6/1 for COVID
        if ($n_date['year'] == '2020')
        {
            ...
            $stmt = $this
                ->conn
                ->getData($query);
            $mkAug31 = mktime(0, 0, 0, 8, 31, $n_date['year']);
            if (empty($stmt) == true)
            {
                if ($make_sure_cycle == 2)
                {
                    ...
                    else if (($n_date[0] > $mkJune1) && ($n_date[0] < $mkAug31))
                    {
                        $cme_title = "CME Payment";
                    }
                    else
    ...
    public function insert_CMEActionHistory($data)
    {
        ...
        else
        {
            ...
            //update the paid document record as checked correctly.
            // $query_checked = "UPDATE tbl_add_cme tac, action_history_cme ahc SET tac.`checked` = 1 WHERE tac.`user_id` = ahc.`user_id`";
            $query_checked = "UPDATE " . $this->table_add_cme . " SET checked = 1 WHERE user_id = " . $_SESSION['user_id'];
            $this
                ->conn
    ...
    //check for already paid or not for COVID to extend by 8/31/2020.
    public function checkCOVIDCME($reciept_title, $cme_cycle_start)
    {
        if ($cme_cycle_start == '2018')
        {
            $n_date = getdate(date("U"));
            if ($n_date[0] > mktime(0, 0, 0, 6, 1, 2020) && $n_date[0] < mktime(0, 0, 0, 8, 31, 2020))
            {
                $action_num = 3;
                $pay_num = 1;
                $cme_cycle_start = 2018;
                $query_cme_paid = "SELECT * FROM " . $this->table_action . " WHERE user_id='" . $_SESSION['user_id'] . "' AND action_num='" . $action_num . "' AND amount_num='" . $pay_num . "' AND cme_cycle_start='" . $cme_cycle_start . "' AND receipt_title='" . $reciept_title . "'";
                $check_cme_paid = $this
                    ->conn
                    ->getCount($query_cme_paid);
                if ($check_cme_paid > 0)
