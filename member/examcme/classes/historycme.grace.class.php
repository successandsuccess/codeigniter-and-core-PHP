...
class PaymentHistoryCME
{
    ...
    public function pay_title($id)
    {
        ...
        //make time for 6/7/year
        $mkJune7 = mktime(0, 0, 0, 6, 7, $n_date['year']);
        ...
        $make_sure_cycle = $n_date['year'] - $pay_cycle;
            ...
            if (empty($stmt) == true)
            {
                if ($make_sure_cycle == 2)
                {
                    ...
                    else if (($n_date[0] > $mkJune7) && ($n_date[0] < $mkAug1))
                    {
    ...
    public function insert_CMEActionHistory($data)
    {
        ...
        else
        {
            ...
            $query_temp = "UPDATE " . $this->table_temp . " SET cme_due = '" . ($data['cme_year'] + 4) . "-06-01', cme_student = 0 WHERE user_id = " . $_SESSION['user_id'];
