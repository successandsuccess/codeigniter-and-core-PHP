...
class AnnouncementObject
{
    ...
    private $conn;
    private $table_name = "announcementlists";
    ...
    {
        $check = "SELECT * FROM " . $this->table_name . " WHERE contents = '" . $this->contents . "' AND subject = '" . $this->subject . "'";
        $check_count = $this
            ->conn
            ->getCount($check);
        if ($check_count > 0)
        {
            $query = "UPDATE " . $this->table_name . " SET created = '" . $this->created . "', audience = '" . $this->audience . "', subject = '" . $this->subject . "', contents = '" . $this->contents . "', visibility = '" . $this->visibility . "', button = '" . $this->button . "' WHERE contents = '" . $this->contents . "' AND subject = '" . $this->subject . "'";
        }
        else
        {
            $query = "INSERT INTO " . $this->table_name . " (created, audience, subject, contents, visibility, button) VALUES ('" . $this->created . "', '" . $this->audience . "','" . $this->subject . "','" . $this->contents . "','" . $this->visibility . "', '" . $this->button . "') ";
        }
        if ($this
            ->conn
            ->execute($query))
    ...
    // read data by audience
    public function readByAudience($audience)
    {
        if($audience == 'CAA') {
            $this->audience = 'CAAs';
        }
        ...
        $query = "SELECT * FROM " . $this->table_name . " WHERE (audience = '" . $this->audience . "' OR audience = 'All') AND visibility = 1 ORDER BY created DESC, id DESC";
        $stmt = $this
            ->conn
            ->getData($query);
        return $stmt;
    ...
    {
        $query = "SELECT * FROM " . $this->table_name . " ORDER BY created DESC, id DESC";
        //print "query:$query<br>\n";
        $stmt = $this
            ->conn
            ->getData($query);
    ...
    {
        $query = "UPDATE " . $this->table_name . " SET created = '" . ($this->created) . "', visibility = '" . ($this->visibility) . "', subject = '" . ($this->subject) . "', contents = '" . ($this->contents) . "', audience = '" . ($this->audience) . "', button = '" . ($this->button) . "' WHERE id = " . $this->id;
        if ($this
            ->conn
            ->execute($query))
        {
    ...
    public function delete()
    {
        if ($this
            ->conn
            ->delete($this->id, $this->table_name))
        {
            return true;
        }
        else
