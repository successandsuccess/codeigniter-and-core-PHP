	...
	_mouseDrag: function(event) {
		...
		for (var i = this.items.length - 1; i >= 0; i--) {
			...
			if(itemElement != this.currentItem[0] //cannot intersect with itself
				...
				//console.log($.ui.isOverAxis(this.positionAbs.left, item.left, item.width));
	...
	serialize: function(o) {
		...
		if(this.options.nested){
			...
			for(var i=0;i<items.length;i++){
				var itemlevel = this._getLevel($(items[i]));
				if(level== itemlevel){
					var res = ($(o.item || items[i]).attr(o.attribute || 'id') || '').match(o.expression || (/(.+)[-=_](.+)/));
	...
	toArray: function(o) {
		...
		if(this.options.nested){
			...
			for(var i=0;i<items.length;i++){
				...
				if(itemlevel == level){
					currentAr.push([$(o.item || item).attr(o.attribute || 'id') || '',[]]);
		...
		if(!this.options.nested){
			...
			if(this.options.maxLevels <= level+childlevels){
				if(this.error != true){
					...
					$('body').css("cursor", "not-allowed"); //Reset cursor
					this.error = true;
				}
			}else{
				if(!$.ui.isOverAxis(this.positionAbs.left, i.left, i.width)){
