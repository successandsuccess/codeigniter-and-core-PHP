...
if($products){
	...
	for($i=0; $i < count($products); $i++){
		...
		if(!empty($products[$i]->state) || !empty($products[$i]->zip_code) || !empty($products[$i]->city)){
    		...
    		$json[$i] = @file_get_contents($file);
    
    		$obj[$i] = json_decode($json[$i], true);
    		
    // 		if(!empty($obj[$i]['results'][0]['geometry']['location']['lat']) && $obj[$i]['results'][0]['geometry']['location']['lng']){
    		    
    // 		    $count_num = $count_num + 1;
    // 		    $count_lat = $count_lat + intval($obj[$i]['results'][0]['geometry']['location']['lat']);
    // 		    $count_lng = $count_lng + intval($obj[$i]['results'][0]['geometry']['location']['lng']);
    		    
    // 		}else{
    		    
    // 		    echo"<center style='font-family:Circe;font-size: 17px;'>There are no results for this search criteria.</center>"; exit;
    		    
    // 		}
    
    		$gps[$i] = $obj[$i]['results'][0]['geometry']['location']['lat'].", ".$obj[$i]['results'][0]['geometry']['location']['lng'];
    		
    		$data[$i] = array("DisplayText"=>$products[$i]->user_id, "ADDRESS"=>$address, "LatitudeLongitude"=>$gps[$i],"MarkerId"=> "Customer");
...
// 	$center_lng = (int)($count_lng/$count_num);
	$center_lat = $obj[0]['results'][0]['geometry']['location']['lat'];
	$center_lng = $obj[0]['results'][0]['geometry']['location']['lng'];
}
...
<div class="super_container search-page" id="super_container">
	<div class="container">
    	<div class="row">
			...
			There <?php if(count($products) > 1) echo"are ".count($products)." CAAs";
			            else echo"is 1 CAA"; 
