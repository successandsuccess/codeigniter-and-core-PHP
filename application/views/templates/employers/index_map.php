...
if($products){
	...
	for($i=0; $i < count($products); $i++){
		...
		if(empty($products[$i]->employer_gps) == false){
		    

            $address = ' ';
            
    		$address = urlencode($products[$i]->employer_state . "+" . $products[$i]->employer_zip . "+" . $products[$i]->employer_city);
    		
    		$gps[$i] = $products[$i]->employer_gps;
    		...
    		$data[$i] = array("DisplayText"=>$products[$i]->id, "ADDRESS"=>$products[$i]->employer_address, "LatitudeLongitude"=>$gps[$i],"MarkerId"=> "Customer");
...
<div class="super_container search-page" id="super_container">
	<div class="container">
    	<div class="row">
			...
			There <?php if($count_num > 1) echo"are ".$count_num." Employers";
			            else echo"is 1 Employer"; 
